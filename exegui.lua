-- Kaino Executor Sex Update Remake made by @chillz_azy (yt)
...
G2L["1e"]["Text"] = "Kaino Executor Sex Update - "..synversion;
...
G2L["4f"]["Active"] = true;
G2L["4f"]["Text"] = [[Kaino Executor Sex Update - Save File]];
G2L["4f"]["Name"] = [[Title]];
...
G2L["63"]["Text"] = [[Kaino Executor Sex Update - Script Hub]];
G2L["63"]["Name"] = [[Title]];
...
G2L["90"]["Active"] = true;
G2L["90"]["Text"] = [[Kaino Executor Sex Update - Open FIle]];
G2L["90"]["Name"] = [[Title]];
...
local function C_49()
	...
	buttons.Execute.MouseButton1Up:Connect(function()
		if not injected then
			title.Text = "Kaino Executor Sex Update - "..synversion.." (not injected! press attach)"
	...
	buttons.Attach.MouseButton1Up:Connect(function()
		if not injected then
			title.Text = "Kaino Executor Sex Update - "..synversion.." (checking...)"
			task.wait(0.8)
			title.Text = "Kaino Executor Sex Update - "..synversion.." (injecting...)"
			task.wait(2.3)
			title.Text = "Kaino Executor Sex Update - "..synversion.." (checking whitelist...)"
			task.wait(1.6)
			title.Text = "Kaino Executor Sex Update - "..synversion.." (scanning...)"
			task.wait(1.3)
			title.Text = "Kaino Executor Sex Update - "..synversion.." (ready!)"
			injected = true
			task.wait(1)
			title.Text = "Kaino Executor Sex Update - "..synversion
		else
			title.Text = "Kaino Executor Sex Update - "..synversion.." (already injected!)"
			task.wait(1)
			title.Text = "Kaino Executor Sex Update - "..synversion
		end
	end)
...
local function C_51()
	...
	script.Parent.savescript.MouseButton1Up:Connect(function()
		...
		makefolder("Kaino")
		writefile("Kaino/"..script.Parent.scriptname.Text..".lua", module:GetActiveTextbox().Frame.Textbox.Text)
	end)
...
local function C_71()
	...
	yes.Background.ScrollingFrame.Dex.MouseButton1Up:Connect(function()
		...
		script.Parent.Description.Text = [[A version of the popular Dex explorer with patches specifically for Kaino Executor Sex Update.]]
	end)
	...
	yes.Background.ScrollingFrame.ScriptDumper.MouseButton1Up:Connect(function()
		...
		script.Parent.Description.Text = [[Saves the game as a .RBXL]]
	end)
	...
	yes.Execute.MouseButton1Up:Connect(function()
		for i,v in pairs(yes:GetChildren()) do
			...
			elseif v.Name == "ScriptDumper" then
				saveplace({FileName = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name})
			end
			end
		end
	end)
...
local function C_84()
	...
	for index, value in pairs(listfiles("Kaino")) do
    	print(value)
		if isfile(value) then
			task.wait(0.05)
            local editedString = string.gsub(value, [[Kaino\]], "")
...
local function C_92()
	...
	script.Parent.savescript.MouseButton1Up:Connect(function()
		...
		if isfile(script.Parent.scriptname.Text) then
			...
			script.Parent.Title.Text = "Kaino Executor Sex Update - Open File (File not found!)"
			task.wait(1)
			script.Parent.Title.Text = "Kaino Executor Sex Update - Open File"
		end
	end)
